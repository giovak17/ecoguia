...
from .forms import UsuarioForm, RetoForm, RecompensaForm
...
def RetosYRecompensas(request):
    ...
    return render(request, 'administradores/RetosYRecompensas.html', {
        'retos': retos,
        'recompensas': recompensas
    })

@login_required(role="administrador")
def agregar_reto(request):
    if request.method == 'POST':
        form = RetoForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('administradores:RetosYRecompensas')
    else:
        form = RetoForm()

    return render(request, 'administradores/FormReto.html', {'form': form})
...
def agregar_recompensa(request):
    ...
    return render(request, 'administradores/FormRecompensa.html', {'form': form})
...
def recreate_pk_sequence_and_sync(db_settings, psql_path, db_pass, table_name, pk_column_name):
    ...
    try:
        ...
        subprocess.run(cmd_clean_pk, capture_output=True, text=True) #
    except Exception:
        pass
    add_pk_sql = f"""
    DO $$
    ...
    try:
        ...
        if "secuencia de bytes no v" not in error_output: 
